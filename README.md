...
Descrição do Projeto:
 ...
 Instruções para configurar e executar a aplicação localmente:
    ...
    - Para ter acesso às funcionalidades que envolvem APIs é necessário trocar a variavel das APIs no ficheiro config.example, encontrado na pasta JavaScript, basta trocar cada variável pela respetiva API, de           seguida é necessário ir onde essas variaveis são chamadas e trocar o caminho delas, de CONFIG.WEATHER_API_KEY para CONFIG.EXAMPLE.WEATHER_API_KEY e o mesmo para outras APIS
...
Descrição das APIs externas utilizadas e como foram integradas:
    ...
    API do OpenStreetMap (Nominatim) e Leaflet:
        Utilizámos a biblioteca open-source Leaflet em conjunto com os dados do OpenStreetMap para criar mapas interativos no nosso site, sem necessidade de chaves de API comerciais.
        A integração divide-se em duas partes:
            - Temos um mapa principal na secção dos contactos que é carregado automaticamente com as coordenadas da Universidade dos Açores, onde fica situado o CACA.
            - Para os eventos, usámos a API de Geocoding do Nominatim. Quando o utilizador clica na localização de um evento, o mapas.js pega no nome desse local e faz um pedido à API do                                         Nominatim, que o traduz em coordenadas geográficas exatas (latitude e longitude). Com esses dados, o código abre um pop-up, desenha o mapa focado nessa zona e coloca um pino vermelho por cima da                   localização. Se o local escrito no evento não existir ou não for encontrado pelo sistema de GPS, o código trata esse erro apresentando uma mensagem de erro no lugar do mapa.
